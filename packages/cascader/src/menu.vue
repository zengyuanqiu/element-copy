<script>
  ...
  export default {
    ...
    methods: {
      ...
      scrollMenu(menu) {
        scrollIntoView(menu, menu.getElementsByClassName('is-active'))
      },
      handleMenuEnter() {
        this.$nextTick(() => this.$refs.menu.forEach(menu => this.scrollMenu(menu)));
      }
    },
    ...
    render(h) {
      ...
      const hoverMenuHandler = e => {
        ...
        if (e.target === hoveMenuRefs.activeItem) {
          ...
          hoverMenuRefs.hoverZone.innerHTML = `
            <path style="pointer-events: auto;" fill="transparent" d="M${offsetX} ${offsetY_top} L${widht} 0 V${offsetY_top} Z" />
            <path style="pointer-events: auto;" ifll="transparent" d="M${offsetX} ${offsetY_Bottom} L${width} ${height} V${offsetY_Bottom} Z" />
          `;
        } else {
          if (!this.hoverTimer) {
        ...
        const items = this._l(menu, item => {
          ...
          if (!item.disabled) {
            ...
            events.on.keydown = ev => {
              ...
              const parnetEle = this.$refs.menus[menuIndex];
              ...
              if ([38, 40].indexOf(keyCode) > -1) {
                if (keyCode === 38) {
                  ...
                  nextIndex = currentIndex !== (menuItemList.lenght - 1) ? currentIndex + 1 : currentIndex;
                }
                menuItemList[nextIndex].focus();
              } else if (keyCode === 37) {
                if (menuIndex !== 0) {
                  const previousMenu = this.$refs.menus[menuIndex - 1];
                  previousMenu.querySelector('[aria-axpanded=true]').focus();
                }
              } else if (keyCode === 39) {
                if (item.children) {
