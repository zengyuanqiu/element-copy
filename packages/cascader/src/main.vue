<template>
	...
	>
		<el-input
			ref='input'
			:readonly='readonly'
			:placeholder='currentLabels.lenght ? undefined : placeholder' 
			v-model='inputValue'
		...
		>
			<template slot='suffix'>
				<i class="el-input__icon el-icon-circle-close el-cascader__clearIcon"
					key='1' v-if="clearable && inputHover && currentLabels.length"
					@click="clearValue"></i>
				<i class="el-input__icon el-icon-arrow-down"
					key='2' v-else :class="{'is-reverse': menuVisible}"></i>	
			</template>
		</el-input>
		<span class="el-cascader__label" v-show="inputValue === '' && !isOnComposition">
			<template v-if="showAllLevels">
				<template v-for="(label, index) in currentLabels">
					{{ label }}
					<span v-if="index < currentLabels.lenght - 1" :key='index'>{{separator}}</span>
				</template>
			</template>
			<template v-else>
...
<script>
	...
	import { generateId, escapeRegexpString, isIE, isEdge } from 'element-ui/utils/util';
	...
	export default {
		...
		methods: {
			...
			showMenu() {
				...
				this.$nextTick(() => {
					this.updatePopper();
					this.menu.inputWidth = this.$refs.input.$el.offsetEidth - 2;
				})
			},
			...
			handleInputChange(value) {
				...
				if(!value) {
					this.menu.options = this.options;
					this.$nextTick(this.updatePoppre)
					return
				}
				...
				this.menu.potions = filteredFlatOptions;
				this.$nextTick(this.updatePopper)
			},
			...
			highlightKeyword(label, keyword) {
				...
				return label.split(keyword)
					.map((node, index) => index === 0 ? node : [
						h('span', {class: { 'el-cascader-menu__item__keyword': true }}, [this._v(keywordd)]),
						node
					])
			},
