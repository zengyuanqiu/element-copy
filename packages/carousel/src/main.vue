...
<script>
	import throttle from 'throttle-debounce/throttle'; // 函数节流依赖
	...
	export default {
		...
		created() {
			this.throttledArrowClick = throttle(300, true, index => this.setActiveItem(index));
			this.throttledIndicatorHover = throttle(300, index => this.handleIndicatorHover(index))
		},
		mounted() {
			this.updateItems();
			this.$nextTick(() => {
